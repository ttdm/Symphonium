    ...
    for (int i = 0; i < midifile->getTrackCount(); i++ )
    {
        for (int j = 0; j < (*midifile)[i].getEventCount(); j++ )
        {
            if ((*midifile)[i][j].isNoteOn())
            {
                ...
                path.addRoundedRect(QRectF(getNoteX0(currNote)-0.5, getHeightForTime(endNoteTime)-0.5, noteSize-1, getHeightForTime(startNoteTime)-getHeightForTime(endNoteTime)-1), 4, 4);
                QPen pen(Qt::black, 2);
                painter->setPen(pen);
